...
class PlayerAI(AbstractPlayer):
    ...
    def encontraPosicoes (self, matriz, M, N, valor):
        posicoes = []
        for i in range(0, M):
            for j in range(0, N):
                if matriz[i][j] == valor:
    ...
    def encontra_estados_sucessores (self, matriz, M, N, posicao_atual):
        i = posicao_atual[0]
        j = posicao_atual[1]
        estados_sucessores = []
    ...
    def apresenta_solucao (self, estado, predecessores, iteracao):
        caminho = []
    ...
    def calcula_distancia_meta (self,estado, estados_finais):
        x = estado[0]
        y = estado[1]
    ...
    def mostra_estados_fila (self, fila):
        print ("===================================")
    ...
    # de cada um deles.
    def mostra_valores_franja (self, franja, heuristica):
        print ("===================================")
    ...
    # Algoritmo: Busca A* (A Estrela)
    def busca_a_estrela (self, matriz, M, N, estado_inicial, estados_finais):
        distancia_meta = {}
        ...
        distancia_meta[estado_inicial] = self.calcula_distancia_meta (estado_inicial, estados_finais)
        ...
        while len(franja) != 0:
            ...
            if estado in estados_finais:
                ...
                break
            estados_sucessores = self.encontra_estados_sucessores(matriz, M, N, estado)
            ...
            for i in range (0, len(estados_sucessores)):
                ...
                if sucessor not in estados_expandidos and sucessor not in franja:
                    ...
                    if sucessor not in heuristica.keys():
                        distancia_meta[sucessor] = self.calcula_distancia_meta(sucessor, estados_finais)
        ...
        if solucao_encontrada == True:
            return self.apresenta_solucao (estado, predecessores, iteracao)
        else:
    ...
    def get_move_a_star (self, matriz, M, N):

        estado_inicial = self.encontraPosicoes (matriz, M, N, '3')
        estados_finais = self.encontraPosicoes (matriz, M, N, '0')
        print (str(matriz))
        print ("Estado Inicial: " + str(estado_inicial))
        print ("Estado Final: " + str(estados_finais))
        return self.busca_a_estrela (matriz, M, N, estado_inicial[0], estados_finais)
